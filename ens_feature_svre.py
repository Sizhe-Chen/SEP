...
def get_args():
    ...
    parser.add_argument('--msvrg', type=int, default=16)
...
def yield_task_parameters(): 
    for tb in [6, 8, 9]: yield '--num_model=15 --msvrg=15 --eps=8 --target_batch=%d' % (tb)
...
if __name__ == '__main__':
    ...
    path = 'samples/' + ('testv_' if args.testloader else 'trainv_') + str(args.num_model)#+ '_'.join([str(x) for x in used_indexes])
