...
def valid_test(net, dataloader, mode, inputs_c, f):
    ...
    for batch_idx, (inputs, targets, index) in enumerate(dataloader):
        ...
        targets_attack = torch.ones(targets.shape, device=device, dtype=targets.dtype) * ((targets + 50) % 100)
...
def yield_task_parameters(): 
    for tb in range(10): yield '--target_batch=%d' % (tb)
...
if __name__ == '__main__':
    ...
    ckpts = ['ckpt/ResNet18/' + x for x in os.listdir('ckpt/ResNet18/') if 'CIFAR100' in x]
    ...
    path = 'samples/' + ('testv100_' if args.testloader else 'trainv100_') + str(args.num_model)#+ '_'.join([str(x) for x in used_indexes])
