...
@torch.no_grad()
def get_mean_feature(net, dataloader):
    ...
    for batch_idx, (inputs, targets, index) in enumerate(dataloader):
        ...
        net(inputs)
        features[index] += net.net.features#net(inputs, return_features=True)
        #meminfo = pynvml.nvmlDeviceGetMemoryInfo(handle)
        #print(batch_idx, meminfo.free / (1024 ** 3))
        #features[index] += net.net.feature
        labels[index] += targets
    features_mean = torch.zeros(10, 512, device=device)
    for i in range(10): features_mean[i] = features[labels == i].mean(0)[None, ...]
...
def valid_test(net, dataloader, mode, inputs_c, f):
    ...
    for batch_idx, (inputs, targets, index) in enumerate(dataloader):
        ...
        with torch.enable_grad():
            ...
            loss = criterion(net.net.features, f[targets_attack]) #+ criterion(net.net.features, features_random_order)
...
def yield_task_parameters(): 
    #for num_model in [5, 30, 120]:
        #for tb in range(10): yield '--num_model=%d --target_batch=%d --eps=2' % (num_model, tb)
    for eps in [4]:
        for num_model in [120]:
            for tb in range(5, 10): yield '--num_model=%d --target_batch=%d --eps=%d' % (num_model, tb, eps)
...
if __name__ == '__main__':
    ...
    path = 'samples/' + ('testf_' if args.testloader else 'trainf_') + str(args.num_model)#+ '_'.join([str(x) for x in used_indexes])
    ...
    for i in range(num_step):
        ...
        for j, ckpt in enumerate(used_ckpts):
            ...
            if len(fs) < len(used_ckpts): fs.append(get_mean_feature(net, loader))
